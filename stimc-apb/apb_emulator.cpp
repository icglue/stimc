...
bool apb_emulator::write (uint32_t addr, uint8_t strb, uint32_t wdata)
{
    apb_clk_en_o = 1;
    apb_sel_o    = 1;
    apb_write_o  = 1;
...
bool apb_emulator::read (uint32_t addr, uint32_t &rdata)
{
    apb_clk_en_o = 1;
    apb_sel_o    = 1;
    apb_write_o  = 0;
...
void apb_emulator::testcontrol ()
{
    /* reset... */
    wait (reset_release_event);
    printf ("reset released\n");
    wait (clk_event);
    wait (clk_event);

    write (0x1234, 0xf, 0xa5a5a5a5);
    wait (clk_event);
    uint32_t rdata;
    read (0x5678, rdata);
    wait (clk_event);
...
void apb_emulator::clock ()
{
    clk_event.trigger ();
}

void apb_emulator::reset_release ()
{
